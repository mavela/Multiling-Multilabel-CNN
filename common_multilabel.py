...
def load_fasttext_data(fn, prefix='', lowercase=True, label_string='__label__'):
    ...
    with open(fn) as f:
        for ln, l in enumerate(f, start=1):
            ...
            fields = l.split("\t")
            if len(fields) != 2:
                raise ValueError('expected two tab-separated fields on line {} in {}'.format(ln, fn))
            labelss, tokens = fields[0], fields[1].split()
   #         print("labels, tokens", labelss, tokens, file=sys.stderr, flush=True)
      ...
      #      print("labels", cleaned_labels, file=sys.stderr, flush=True)
            labels_list.append(cleaned_labels)
  #  print("list", labels_list, file=sys.stderr, flush=True)
    #mlb = MultiLabelBinarizer()
#    labels_bin = mlb.fit_transform(labels_list)
 #   print("bin", labels_bin, file=sys.stderr, flush=True)
  #  classes = len(mlb.classes_)
    return texts, labels_list#_bin, mlb #classes) 


def transform2mlb(labels):
    mlb = MultiLabelBinarizer()
    labels_bin = mlb.fit_transform(labels)
    return labels_bin,mlb


def transform2mlb_val(v_labels,mlb):
    labels_bin = mlb.transform(v_labels)
    return labels_bin
...
#def binarize_validation_labels(labels):
#    mlb
...
#def label_to_index(labels, label_to_idx):
    
#    for label in labels:
#        if label not in label_to_idx:
#            label_to_idx[label] = leb(label_to_idx)
